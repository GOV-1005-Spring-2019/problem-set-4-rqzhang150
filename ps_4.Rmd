...
ans_q1_5 <- round(as.numeric(rep_earliest - dem_earliest))
...
weighted_all <- poll %>% 
  filter(educ != "[DO NOT READ] Refused") %>% 
  select(response, educ, file_party, final_weight) %>% 
  group_by(educ, file_party, response) %>% 
  summarize(total_weight = sum(final_weight)) %>% 
  spread(key = response, value = total_weight) %>% 
  mutate_at(vars(-group_cols()),list(~replace_na(.,0))) %>% 
  mutate(total_weight = `3` + Dem + Rep + Und)
  
dem_change <- weighted_all %>% 
  filter(file_party == "Democratic") %>% 
  mutate(changed_weight = `3` + Rep) %>% 
...
bind_rows(dem_change, rep_change) %>% 
